...
void show_solution(struct Box ** board,struct Box ** copy_board, struct Perso * tab_perso, struct Perso * tab_perso_jeu,struct Clue ** tab_clue){
    ...
    while(!WindowShouldClose()){
        ...
        if(b != 0){
            game_start(copy_board, tab_perso_jeu, tab_clue, spritesheet) ;
        }
        else{
    ...
    if(IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
        ...
        if(clue_x >= 0 && clue_x < NUMBER_OF_CHARACTERS && clue_y >= 0 && clue_y < 1){
            ...
            if(i >= 0 && i < BOARD_SIZE && j >= 0 && j < BOARD_SIZE){
                unplace_a_character(tab_perso, perso_selectionne, copy_board) ; // retire l'ancienne position avant de replacer
                place_a_character(i, j, tab_perso, perso_selectionne, copy_board) ;
            }
        }
    }
}